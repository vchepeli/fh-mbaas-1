...
var dns = require('dns');
var checkApps = require('./health/apps.js').checkAppsStatus;
...
function initFhHealth() {
  ...
  if (fhconfig.value('openshift3')) {
    ...
    fhhealth.addCriticalTest('Check apps reachable', checkApps);
  }

  return fhhealth;
}

function checkIfAppsAreReachable(apps, cb) {
...
// }
//
// function checkAppStatus(appData, appName) {
//
//
//
//
...
//   //
//   //   if (err) {
//   //     logger.error('Error invoking endpoint', {params:params, err: err});
//   //     return cb(new ResultBuilder()
//   //       .withId(id)
//   //       .withStatus(STATUSES.TYPES.CRITICAL)
//   //       .withError(err)
//   //       .build());
//   //   }
//   //
//   //   if (response.statusCode !== 200) {
//   //     logger.error('Error invoking endpoint', {params:params, err: response.statusMessage, code: response.statusCode});
//   //     return cb(new ResultBuilder()
//   //       .withId(id)
//   //       .withStatus(STATUSES.TYPES.CRITICAL)
//   //       .withError({code: response.statusCode, message: errorMessage})
//   //       .build());
//   //   }
//   //
//   //   var res = JSON.parse(body);
//   //   if (res.status === STATUSES.TYPES.OK) {
//   //     return cb(null, new ResultBuilder()
//   //       .withId(id)
//   //       .withStatus(STATUSES.TYPES.OK)
//   //       .withMessage(okMessage)
//   //       .build());
//   //   }
//   //
//   //   return cb(new ResultBuilder()
...
function checkDNS(callback) {
  dns.resolve('google.com', function(err, addresses) {
    if (err) {
      return callback(result('DNS', 'error', util.inspect(err)));
    }

    return callback(null, result('DNS', 'OK', null));
  });
}
